...
int main(int argc, char *argv[])
{
    ...
    // add local ip to multi ip
    struct ip_mreq group;
    group.imr_multiaddr.s_addr = inet_addr(MULTI_IP);
    group.imr_interface.s_addr = htonl(INADDR_ANY);

   ret =  setsockopt(srv_fd, IPPROTO_IP, IP_ADD_MEMBERSHIP, &group, sizeof(group));
    if(ret < 0)
    {
	perror("setsockopt multi");
	exit(1);
    }
