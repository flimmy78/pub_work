...
/*功能：从sour_file文件的offset偏移处开始将count个
 * 字节数据复制到dest_file文件*/
int myfilecopy(const char*sour_file,const char*dest_file,int offset, int count, int copy_mode){
    ...
    while((read(in_file, &buff_uint, 1) == 1) && (counter < count)){
        write(out_file, &buff_uint, 1);counter++;
    }
...
int custom(int need){
    ...
    if((fd = open(fifo_file, O_RDWR)) < 0){
        printf("Function myfilecopy error in source_file:");return (-1);
    }
...
{
    int customer_capactivity = 10;

    if(argv > 1){/*第一个参数指定需要消费的资源数目，默认值为10*/
        sscanf(argv[1],"%d",&customer_capactivity);
    }

    if(customer_capactivity > 0){
        custom(customer_capactivity);
    }
